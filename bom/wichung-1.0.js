...
function _getStyle( elem, name ) {//text1.html-align, textAlign
    ...
    // Or the W3C’s method, if it exists
        ...
        // It uses the traditional ‘text1.html-align’ style of rule writing,
        //   instead of textAlign
        name = name.replace(/([A-Z])/g, "-$1");// textAlign =>  text1.html-Align
        name = name.toLowerCase(); //text1.html-Align => text1.html-align
...
function text(e, txt) {
    ...
    if(!txt){
        ...
        for ( var j = 0; j < e.length; j++ ) {
            // If it’s not an element, append its text1.html value
            // Otherwise, recurse through all the element’s children 
            t += e[j].nodeType != 1 ?
                e[j].nodeValue : text(e[j].childNodes);
        }

        // Return the matched text1.html
        return t;
    }
